...
flags.DEFINE_string('video', './data/test.mp4', 'path to input video or set to 0 for webcam')
...
def main(_argv):
    ...
    while grabbed:
        ...
        for track in tracker.tracks:
            ...
            total_count.add(track.track_id)
            bbox = track.to_tlbr()
        ...
        # draw bbox on screen
            ...
            x1 = int(3*width/6+width/4.5)
            y1 = 0
            x2 = int(3*width/6+width/4.5)
            y2 = height
            cv2.line(frame, (x1, y1), (x2, y2), (0, 255, 0), thickness=2)

            center_x = int(((bbox[0])+(bbox[2]))/2)

        # add id to left_count if person is to the left of the line
            if center_x < x1:
                left_count.add(track.track_id)
                frame_set.add(track.track_id)

        # add id to right_count if it is to the right of the line
            if center_x >= x1:
                right_count.add(track.track_id)
    ...
    print("Unique persons = ", len(total_count))
    left_count = left_count - frame_set
    print("Left to Right person = ", len(left_count.intersection(right_count)))
